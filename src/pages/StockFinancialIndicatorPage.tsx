...
import { importStockFinancialIndicatorCsv } from '../api/stockFinancialIndicator'
import type {
  StockFinancialIndicatorFolderImportResult,
  StockFinancialIndicatorImportResult,
} from '../types'
import {
  importStockFinancialIndicatorFolderConcurrent,
  type FolderImportProgress,
} from '../utils/stockFinancialIndicatorImport'
...
function ImportResultCard({
  ...
  title: string
  result: StockFinancialIndicatorImportResult | StockFinancialIndicatorFolderImportResult
}) {
...
export default function StockFinancialIndicatorPage() {
  const queryClient = useQueryClient()
  const folderInputRef = useRef<HTMLInputElement>(null)
  const abortRef = useRef<AbortController | null>(null)

  const [singleResult, setSingleResult] = useState<StockFinancialIndicatorImportResult | null>(null)
  const [folderResult, setFolderResult] = useState<StockFinancialIndicatorFolderImportResult | null>(null)
  const [folderImporting, setFolderImporting] = useState(false)
  const [folderProgress, setFolderProgress] = useState<FolderImportProgress | null>(null)

  const invalidateQueries = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['stock-financial-indicator'] })
    queryClient.invalidateQueries({ queryKey: ['stocks'] })
  }, [queryClient])

  const importMutation = useMutation({
    mutationFn: importStockFinancialIndicatorCsv,
    onSuccess: (result) => {
  ...
  const handleFolderSelect = useCallback(
    async (fileList: FileList | null) => {
      ...
      try {
        const result = await importStockFinancialIndicatorFolderConcurrent(
          Array.from(fileList),
          setFolderProgress,
          controller.signal,
        )
  ...
  return (
    <div>
      <Typography.Title level={3}>财务指标数据导入</Typography.Title>
        ...
        <Col xs={24} lg={12}>
          <Card title="单文件导入" style={{ marginBottom: 16 }}>
            <Dragger {...singleUploadProps}>
              ...
              <p className="ant-upload-hint">示例：600036.SH.csv</p>
            </Dragger>
          ...
          <Card title="文件夹批量导入">
            ...
            >
              <Dragger disabled={busy} openFileDialogOnClick={false} showUploadList={false}>
                ...
                <p className="ant-upload-hint">支持批量导入，自动分批顺序上传（每批最多 16 个文件）</p>
            ...
            {folderImporting && folderProgress && (
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ marginTop: 8 }}>
                  <Button size="small" icon={<StopOutlined />} onClick={handleCancelFolderImport}>
                    取消导入
                  </Button>
                </div>
              </div>
