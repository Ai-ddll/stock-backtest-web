...
import { importStockDividendCsv } from '../api/stockDividend'
import type { StockDividendFolderImportResult, StockDividendImportResult } from '../types'
import {
  importStockDividendFolderConcurrent,
  type FolderImportProgress,
} from '../utils/stockDividendImport'
...
function ImportResultCard({
  ...
  title: string
  result: StockDividendImportResult | StockDividendFolderImportResult
}) {
...
export default function StockDividendPage() {
  ...
  const [singleResult, setSingleResult] = useState<StockDividendImportResult | null>(null)
  const [folderResult, setFolderResult] = useState<StockDividendFolderImportResult | null>(null)
  const [folderImporting, setFolderImporting] = useState(false)
  const [folderProgress, setFolderProgress] = useState<FolderImportProgress | null>(null)

  const invalidateQueries = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['stock-dividend'] })
    queryClient.invalidateQueries({ queryKey: ['stocks'] })
  }, [queryClient])

  const importMutation = useMutation({
    mutationFn: importStockDividendCsv,
    onSuccess: (result) => {
  ...
  const handleFolderSelect = useCallback(
    async (fileList: FileList | null) => {
      ...
      try {
        const result = await importStockDividendFolderConcurrent(
          Array.from(fileList),
          setFolderProgress,
          controller.signal,
        )
  ...
  return (
    <div>
      <Typography.Title level={3}>分红送股数据导入</Typography.Title>
