import { importStockDividendFolder } from '../api/stockDividend'
import type { StockDividendFolderImportResult } from '../types'
...
function createEmptyFolderResult(totalFiles: number, ignoredFiles: number): StockDividendFolderImportResult {
  return {
    totalFiles,
    successFiles: 0,
    failedFiles: 0,
...
function mergeFolderResult(target: StockDividendFolderImportResult, source: StockDividendFolderImportResult) {
...
export async function importStockDividendFolderConcurrent(
  files: File[],
  onProgress?: (progress: FolderImportProgress) => void,
  signal?: AbortSignal,
): Promise<StockDividendFolderImportResult> {
  ...
  try {
    for (const batch of batches) {
      ...
      const batchResult = await importStockDividendFolder(batch)
